...
Since the fundamental motivations behind RandomHash2 follow from [RandomHash][1], only the changes and motivations of moving from RandomHash1 to RandomHash2 are discussed in this document. The reader is advised to familiarize themselves with [PIP-0009][1] for full context. 
...
In validation mode, RandomHash is simply used to hash a block-header in order to verify it's the correct block in the blockchain -- just the same as how SHA2-256D is used in Bitcoin.

In mining mode, RandomHash is used to mine the next block by enumerating nonces in a block header and hashing to determine if an acceptable Proof-of-Work was found. In this mode, RandomHash exploits the partially calculated dependent nonces by resuming them in subsequent hashing (since they were partially evaluated in previous nonces).  Currently, this mode operates at twice the hashrate as validation mode, and is the basis for the "CPU Bias" which helps break GPUs. It is biased towards CPU's since re-using partially calculated nonces is a serial optimization which cannot be efficiently exploited in a parallelized, batch-computation context such as GPU mining. In other words, the optimal nonce-set is enumerated and cannot be ranged for parallel mining as GPUs do (thus optimal for CPU).
...
Since hashing starts and ends with a ```SHA2-256``` the cryptographic strength of RandomHash2 is **at least** that of ```SHA2-256D``` as used in Bitcoin. Assuming the data transformation in between were cryptographically insecure, it would change this minimum security guarantee.

However, the transformation in between is not weak as it involves the use of 18 other cryptographically strong hash algorithms. RandomHash2 is orders of magnitude more cryptographically secure than almost all other hashing algorithms, however it pays for this with massive performance penalty (which is intentional).

Within the 18 hash algorithms use, some are considered cryptographically weak such as MD5. The use of some weak algorithms is inconsequential to overall security and their purpose is to introducing ASIC-complexity. 

RandomHash2's structure is essentially a set of nested hashes, structured as follows:
```
RandomHash2(Data) = SHA2_256( H1( H2( .... H_N( SHA2_256(DATA)) ...) )
where
   H_i = a randomly selected hash function based on the output of H_(i-1)   
   N = a random number which is a function of the nonce in DATA and dependent nonces of DATA (not pre-calculable and bound)
```

To shed light on the the weakest possible cryptographic security for some block header called ```WeakestDigest```, we assume it's nonce requires ```MIN_N=2``` levels, and each of those 2 levels has 0 nieghbouring nonce dependencies. Also, we assume the hash algorithms are ```MD5```, the weakest of set. In this case, the cryptographic security can be modeled as:
```
RandomHash2(WeakestDigest) = SHA2_256( MD5 ( MD5 ( SHA2_256( WeakestDigest ) ) ) )
```

As it shows above, this is still far stronger than almost all other cryptocurrencies!
...
RandomHash is memory-light in order to support low-end hardware.  A CPU will only need 300KB of memory to verify a hash. Unlike RandomHash 1, mining does not consume additional memory since the cached nonces are fully evaluated.
