...
Since the fundamental motivations behind RandomHash2 follow from [RandomHash][1], only the changes and motiviations of moving from RandomHash1 to RandomHash2 are discussed in this document. The reader is advised to familiraize themselves with [PIP-0009][1] for full context. 

In short, RandomHash2 is proposed to address the the need for faster validation hashrate of the PascalCoin blockchain. 


## Background 

Whilst RandomHash appears to have empirically achieved it's GPU and ASIC resistivity goals, it's computationally-heavy nature has resulted in an unforseen consequence of slow full-chain validation times.

First of all, RandomHash introduces the concept of nonce-dependency between nonces. This means to evaluate a nonce, the partial evaluation of other random neighbouring nonces is required. This allows RandomHash to operate in two modes, validation mode and mining mode. 

In validation mode, RandomHash is simply used to hash a block-header in order to verify it's consistent with what the next block referenced -- just the same as how SHA2-256D is used in Bitcoin.

In mining mode, RandomHash is used to mine a block header to find a suitable Proof-of-Work for the next block. Here, RandomHash exploits the partially calculated dependent nonces by re-using them in the nonce finding process. Currently, this mode operates at twice the hashrate as validation mode, and is the basis for "CPU Bias". It is biased towards CPU's since re-using partially calculated nonces is a serial optimization which cannot be efficiently exploited in a parallelized, batch computational context such as GPU mining. In other words, the optimal nonceset is enumerated and cannot be pre-determined, thus optimal for CPU.

However, on a typical desktop computer in 2019 the validation rate of RandomHash is approximately 20 hashes per second. At 288 blocks per day, that's 14 seconds to validate a days worth of block-headers and over 1 hour to validate 1 year of blockheaders. Whilst multi-threading, performance tuning has (and can further) massively optimize this performance oversight, it remains a fundamental a long-term issue for archival nodes which maintain the full-chain rather than the last 100 blocks.

RandomHash2 offers order of magnitude improvement in both validation hashrate and mining hashrate. In RandomHash2, the same machine validates at 334 hashes per second yet mines at 1,111 hashes per second with far less memory-hardness. Whilst not empirically tested, these numbes suggest a 333% CPU bias.
...
RandomHash2 is similarly structured to RandomHash, but has some key differences. The below overview outlines the full algorithm, but only the specific differences from RandomHash are provided in this document. 
...
12. Since hashing starts and ends with a ```SHA2-256```, RandomHash2's cryptographic strength is **at least** that of ```SHA2-256D``` as used in Bitcoin and traditional cryptocurrency's.
...
By forcing a nonce evaluation to have at least 2 levels of computation, the miner necessarily requires the full algorithm implementation which prevents simplfied ASIC miners. Also, since each nonce requires at least 2 levels of evaluation, and each of those levels depends on at least 2 more nonces evaluated to at least 1 level each, the number of computations saved by nonce-scanning is balanced by the higher pre-computed cached nonces a miner has by honestly mining without nonce-scanning (due to higher number of dependent neighbouring nonces).
...
As the above table shows, nonce-scanning (via CPU) yields a hashrate penalty, not a benefit. In the opinion of the author, it is unlikely a future implementation optimization would necessarily change this result since it would improve all scanning levels proportionally. However, a line of enquiry is to investigate whether or not the reduced memory-hardness may yield a benefit for GPU-based nonce-scanning.
 
### RandomHash pseudo-code------TODO
```pascal
    const
        HASH_ALGO = [
            SHA2_256
