...
5. The input at round ```x``` is a compression of the transitive closure of all inputs to ```100 bytes```
6. The output of every round is expanded for memory-hardness
7. Randomness is generated using ```Mersenne Twister``` algorithm
8. The final round is then hashed again via ```SHA2_256```, in keeping with traditional cryptocurrency approaches.
...
```pascal
    const
        ...
        begin
            ...
            Result := SHA2_256( AllOutputs[ Length(AllOutputs) - 1 ] )
        end
        ...
        begin
            ...
            if Round = 1 then
                let gen = RandomNumberGenerator(blockHeader)  // can hash blockHeader first, but not required
                let input = blockHeader
            else
                let parentOutputs = RandomHash(blockHeader, Round - 1)
                let gen = RandomNumberGenerator(parentOutputs[Length(parentOutputs) - 1])
                ...
                gen = RandomNumberGenerator(neighborOutputs[Length(neighborOutputs) - 1])
        ...
        Function RandomNumberGenerator(seed : ByteArray) : TMersenneTwister
